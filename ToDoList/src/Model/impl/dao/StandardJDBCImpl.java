...
public class StandardJDBCImpl<E extends Standard> implements IContaTarefa<E>{

	private static Connection connection;
	
	public StandardJDBCImpl(Connection connection) {
		...
		if(conta instanceof Standard) {
			if(id != null && id > 0) {
				try {
					...
					if(tarefa.getIdStandard().equals(conta.getId())) {
	...
	public Tarefa findById(E conta, Integer id) throws queryFailedException {
		...
		try {
			...
			Tarefa tarefa = iTarefa.selectTarefaById(id);
			if(conta instanceof Standard) {
				if(conta.getId().equals(tarefa.getIdStandard())) {
					return tarefa;
				}
			}
		} catch (SQLException e) {
	...
	public List<Tarefa> findAll(E conta) {
		...
		try {
			...
			if(conta instanceof Standard) {		
				Predicate<Tarefa> filterTarefas = tarefa -> { return tarefa.getIdStandard().equals(conta.getId()); };
	...
	public List<Tarefa> findIsConcluida(E conta){
		...
		try {
			...
			if(conta instanceof Standard) {
				Predicate<Tarefa> filterIsEquals = tarefa -> { return tarefa.getIdStandard().equals(conta.getId()); };
